...
def get_song_detail(song_id):
    ...
    # 정보 가져올 페이지
    url_singer_page = 'http://www.melon.com/album/detail.htm?albumId=10115186'

    # 실시간 1~100위 웹페이지 HTML을 data/singer_page.html 에 저장
    file_path = os.path.join(path_data_dir, 'singer_page.html')
    try:
        # refresh_html매개변수가 True일 경우, wt모드로 파일을 열어 새로 파일을 다운받도록 함
        file_mode = 'wt' if refresh_html else 'xt'
        with open(file_path, file_mode) as f:
            response = requests.get(url_singer_page)
            source = response.text
            f.write(source)
    # xt모드에서 있는 파일을 열려고 한 경우 발생하는 예외
    except FileExistsError:
